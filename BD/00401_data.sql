...
insert into store.enlaces (padre_id,nivel,orden,nombre,imagen,ruta,ayuda,usuario) values 
((SELECT id FROM store.enlaces WHERE nombre='MIS ANUNCIOS'),2,20,'Admin. Anuncios','fa-id-card', '#/adminAnuncios', '','rpmamani'),
((SELECT id FROM store.enlaces WHERE nombre='MIS ANUNCIOS'),2,20,'Mi Perfil','fa-address-book-o', '#/perfilUser', '','rpmamani');
...
((SELECT id FROM store.roles WHERE nombre='ANUNCIOS'),(SELECT id FROM store.enlaces WHERE nombre='Mi Perfil'),'rpmamani');



insert into store.usuarios(cuenta, alias, clave,usuario) values 
('daparicio', 'DALI APARICIO', crypt('12345', gen_salt('bf')), 'rpmamani');
