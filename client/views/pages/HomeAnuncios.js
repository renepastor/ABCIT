...
let HomeAnuncios = {
        ...
        <div class="row px-3">
            <div class="col">
                <div class="input-group mb-0 border border-primary">
                    ...
                    <input type="search" id="valSearch" class="form-control" placeholder="Buscar Empleo.." aria-label="Buscar.." aria-describedby="basic-addon2">
                    <div class="input-group-append">
                        <button class="btn btn-outline-primary" type="button" id="btnSearch"><i class="fa fa-search"></i></button>
                    </div>
                </div>
                <ul class="row m-0 p-0 csForm overflow-auto" id="pnlAnuncio">
                </ul>
                <div class="row bg-info text-dark m-0 p-1" id="pnlAnunciosFoot">
                </div>
            </div>
        </div>
        `
    ...
    , after_render: async () => {
        $(document).ready(function(){
            $("#pnlAnuncio").on("mouseenter", ".card",function(){
                if($(this).find(".act").hasClass("csHidden"))
                $(this).find(".act").removeClass("csHidden");
            });
            $("#pnlAnuncio").on("mouseleave", ".card",function(){
                if(!$(this).find(".act").hasClass("csHidden"))
            ...
            $("#pnlAnuncio").height((document.getElementsByTagName("body")[0].offsetHeight - 186));
